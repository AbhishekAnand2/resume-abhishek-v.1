...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ensure resumeData exists from data.js
    if (typeof resumeData !== 'undefined') {
        ...
        if (workContainer && resumeData.workHistory) {
            let workHTML = '';
        ...
        const renderListTemplate = (containerId, dataArray) => {
            const container = document.getElementById(containerId);
            if (container && dataArray) {
                container.innerHTML = dataArray.map(item => `<li>${item}</li>`).join('');
            }
        };
        renderListTemplate('skills-container', resumeData.skills);
        renderListTemplate('certifications-container', resumeData.certifications);
        renderListTemplate('personal-container', resumeData.personalInfo);
        renderListTemplate('languages-container', resumeData.languages);

        // Render Websites
        const webContainer = document.getElementById('websites-container');
        if (webContainer && resumeData.websites) {
        ...
        if (projectsContainer && resumeData.projects) {
            let projectsHTML = '';
        ...
        if (timelineContainer && resumeData.timeline) {
            let timelineHTML = '';
            resumeData.timeline.forEach(node => {
                // Auto-detect briefcase vs graduation cap based on content or format
